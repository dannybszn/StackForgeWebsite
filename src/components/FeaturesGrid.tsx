...
const FeaturesGrid: React.FC = () => {
  useEffect(() => {
    ...
    // Listen for both scroll and touchmove for better mobile support
    window.addEventListener('scroll', handleScroll, { passive: true })
    window.addEventListener('touchmove', handleScroll, { passive: true })
    window.addEventListener('touchend', () => {
      // Ensure blur is removed after touch ends
      clearTimeout(scrollTimeout)
      scrollTimeout = setTimeout(() => {
        isScrolling = false
        glassElements.forEach(element => {
    ...
    function handleMouseMove(this: HTMLElement, e: MouseEvent) {
      ...
      const filter = document.querySelector('#glass-distortion-features feDisplacementMap')
      if (filter) {
  ...
  return (
    ...
    >
      ...
      <svg style={{ display: 'none' }}>
        <filter id="glass-distortion-features">
          ...
          {features.map((feature) => {
            ...
            return (
              <article
                ...
                className="glass-card-dark group relative transition-all duration-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#6D28D9] hover:scale-[1.02] hover:-translate-y-1 w-full h-full min-h-[220px]"
              >
